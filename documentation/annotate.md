...
`v-annotate.pl` takes as input two command line arguments, a
FASTA-formatted sequence file and the name of the output directory you
want it to create and populate with output files.

Here is an example `v-annotate.pl` command using the sequence file
`noro.9.fa` that is included in the VADR distribution in the
`testfiles` subdirectory:

```
v-annotate.pl $VADRSCRIPTSDIR/testfiles/noro.9.fa va-noro.9
```

This command will use the default VADR model library (CM file
...
populate it with [many output files](formats.md).
