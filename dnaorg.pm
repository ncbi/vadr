...
sub initializeHardCodedFTableErrorExceptions { 
  ...
  $reqd_str = "m5e";
  $alwd_str = "olp,aja,ajb,nm3,nop,mtr"; # 'nop' allowed so we can output predictions for features with >= 1 models (e.g. 2 exons) for which >= 1 of the models had a nop
  $misc_feature = 0;
  $start_carrot = 0;
  $stop_carrot  = 0;
  $pred_stop    = 0;
  ...
  $reqd_str = "m3e";
  $alwd_str = "olp,aja,ajb,stp,nst,nm3,nop,mtr"; # 'nop' allowed so we can output predictions for features with >= 1 models (e.g. 2 exons) for which >= 1 of the models had a nop
  $misc_feature = 0;
  $start_carrot = 0;
  $stop_carrot  = 0;
  $pred_stop    = 0;
...
#                "incompat":      string that lists all error codes (separated by commas) the current error code is incompatible with
#                "requires":      string that lists all error codes (separated by commas) the current error code requires
#
#             If @{exceptions_AR} is non-empty, then keys in 
