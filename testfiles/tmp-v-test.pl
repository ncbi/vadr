  ...
  while(my $line = <IN>) { 
    if(($line !~ m/^\#/) && ($line =~ m/\w/)) { 
      ...
      if($line =~ s/^command\:\s+//) { 
        ...
        print("looking for $expfile");
        if(-e "/net/intdev/oblast01/infernal/git/nawrockie/vadr/testfiles/profile-build/test1_singlepos.cds.fa") { printf("\n\nHEYA1\n\n")}
        if(-e "$VADRSCRIPTSDIR/testfiles/profile-build/test1_singlepos.cds.fa") { printf("\n\nHEYA2\n\n")}
