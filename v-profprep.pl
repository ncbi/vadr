...
my $usage         = "Usage: $executable [-options] <sequence metadata tsv> <path to output directory to create>\n";
...
# TODO: We need the seed model length to filter >90%. For now, assume a placeholder or add a command-line arg later.
my $seed_model_len = 0; # Placeholder
...
my %candidate_AH = ();
parse_and_filter_metadata($meta_tsv, $seed_model_len, \%candidate_AH);
...
sub parse_and_filter_metadata {
  my ($tsv_file, $seed_model_len, $candidate_AHR) = @_;
  ...
  ofile_OutputString(\*STDOUT, 1, sprintf("# Parsed %d total sequences from TSV.\n", $total_seqs));
  if ($seed_model_len > 0) {
    ofile_OutputString(\*STDOUT, 1, sprintf("# Retained %d sequences passing length filter (>90%% of seed length %d).\n", $kept_len_seqs, $seed_model_len));
  }
  ofile_OutputString(\*STDOUT, 1, sprintf("# Found %d distinct serotype/genotype groups.\n", scalar(keys %{$candidate_AHR})));
  ...
  ofile_OutputString(\*STDOUT, 1, sprintf("# Tier 1 Filter: Selected %d total sequences (max %d per group) for tier 2 processing.\n", $total_selected, $max_per_group));
